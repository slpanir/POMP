...
class Operation(ABC):
    ...
    def get_predecessors(self) -> List[Operation]:
        """
        Returns the predecessors of the operation.
    ...
    def del_predecessor(self, operation: Operation) -> None:
        """
        Delete a preceding operation and update the relationships.

        :param operation: The operation to be deleted as a predecessor.
        :type operation: Operation
        """
        self.predecessors.remove(operation)
        operation.successors.remove(self)

    def del_successor(self, operation: Operation) -> None:
        """
        Delete a succeeding operation and update the relationships.

        :param operation: The operation to be deleted as a successor.
        :type operation: Operation
        """
    ...
    def _execute(
        ...
        if self.combined_scoring:
            ...
            if self.scoring_function is not None:
                ...
                if 'lock' in kwargs.keys() and kwargs['lock'] is not None:
                    with kwargs['lock']:
            ...
            for thought, score in zip(previous_thoughts, scores):
                ...
                try:
                    new_thought.state["score"] = score
                    # new_thought.state["previous"] = new_thought.state['current']
                    # if new_thought.state['pseudo']:
                    if score < 0.0:
                        if new_thought.state['pseudo']:
                            new_thought.state['current'][2] = new_thought.state['previous'][2]
                        else:
                            new_thought.state['current'][2] = new_thought.state['original'][2]
                except:
                    pass
...
class Generate(Operation):
    ...
    def __init__(
        self, num_branches_prompt: int = 1, num_branches_response: int = 1, lang: str = None,
    ) -> None:
        """
    ...
    def _execute(
        ...
        for thought in previous_thoughts:
            try:
                thought.state["previous"] = thought.state["current"]
            except:
                pass
            base_state = thought.state
            try:
                if base_state['method'] == 'got':
                    base_state['aux_lang'] = [base_state['similarity'].pop(0)[0]]
                else:
                    base_state['aux_lang'] = [self.lang]
            except:
                pass
            # if base_state['pseudo']:
...
class Aggregate(Operation):
    ...
    def _execute(
        ...
        base_state: Dict = {}
        for thought in sorted(previous_thoughts, key=lambda thought: thought.state['score']):
            # try:
            #     thought.state['previous'] = thought.state['current']
            # except:
            #     pass
            base_state = {**base_state, **thought.state}
        previous_thoughts = sorted(
...
class KeepBestN(Operation):
    ...
    def get_best_n(self) -> List[Thought]:
        ...
        try:
            if all(thought.score == 0.0 for thought in previous_thoughts):
